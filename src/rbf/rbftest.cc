...
void printLargeRecord(const int index)
{
    ...
    for(int i = 0; i < 10; i++)
    {
        ...
        // compute the integer
        cout << "attr" << cnt++ << "(int): " << index << endl;
        offset += sizeof(int);

        // compute the floating number
        float real = (float)(index + 1);
        cout << "attr" << cnt++ << "(real): " << real << endl;
        offset += sizeof(float);
    }
    cout << "=========Correct Data Ends===========" << endl;
}
...
int RBFTest_5(PagedFileManager *pfm)
{
    ...
    for (unsigned i = 0; i < PAGE_SIZE; i++) {
        char c1 = *((char *)data + i);
        char c2 = *((char *)buffer + i);
        if (c1 != c2) {
            cout << "Diff: i = " << i << " is " << (int) c2 << " but should be " << (int) c1 << endl;
        }
    }
...
//    cout << "Before createRecordDescriptor(), fileHandle,fileName: " << fileHandle.getFileName()
//         << ", file page # " << fileHandle.getNumberOfPages() << ", file ptr: " << fileHandle.getFilePointer() << endl;
    createRecordDescriptor(recordDescriptor);
//    cout << "After createRecordDescriptor(), fileHandle,fileName: " << fileHandle.getFileName()
//         << ", file page # " << fileHandle.getNumberOfPages() << ", file ptr: " << fileHandle.getFilePointer() << endl;

    // Insert a record into a file
//    cout << "Before prepareRecord(), fileHandle,fileName: " << fileHandle.getFileName() << endl;
    prepareRecord(6, "Peters", 24, 170.1, 5000, record, &recordSize);
//    cout << "After prepareRecord(), fileHandle,fileName: " << fileHandle.getFileName() << endl;
//    cout << "Insert Data:" << endl;
    rbfm->printRecord(recordDescriptor, record);

    rc = rbfm->insertRecord(fileHandle, recordDescriptor, record, rid);
    cout << "Return from insertRecord: " << rc << endl;
    assert(rc == success);
...
//            printLargeRecord(i);
            ...
            cout << "Now compare byte by byte: (total: " << sizes[i] << ")" << endl;
            for (int k = 0; k < sizes[i]; k++) {
                char rt = *((char *)returnedData + k);
                char og = *((char *)record + k);
                if (rt != og) {
                    cout << k << ": is " << hex << (unsigned short) rt << ", should be " << (unsigned short) og << dec << endl;
                }
            }
        } else {
//            cout << "Succeeded when i = " << i << endl;
        }
    }
    if (wrongTime != 0) {
        cout << "Total wrong time: " << wrongTime << endl;
        cout << "Wrong at: ";
        for (size_t i = 0; i < wrongAt.size(); i++) {
            cout << wrongAt[i] << " ";
        }
        cout << endl;
